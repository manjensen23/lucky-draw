...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          // Re-fetch everything to guarantee fresh schema including background_url
          fetchInitialData();
        }
      )
      .subscribe();
  ...
  const fetchParticipants = async (isAllGroups: boolean, targetGroups: string[]) => {
    // 1. Fetch eligible for draw
    let query = supabase.from('participants').select('*');
    if (!isAllGroups) {
      if (targetGroups.length > 0) {
        query = query.in('group', targetGroups);
      } else {
        // If not all groups, but no specific target group is selected, force empty result mathematically
        query = query.eq('group', 'UNMATCHABLE_GROUP_FLAG_xyz');
      }
    }
    const { data: eligibleData, error: eligibleError } = await query;
    if (!eligibleError) {
      setParticipants(eligibleData || []);
    }

    // 2. Fetch total absolute count explicitly
              ...
              <div className="w-full md:w-56">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setWinnerCountOverride(isNaN(val) ? 1 : Math.max(1, val));
                  }}
